...
const fetchDashboardData = async ()=>{
    try {
        setLoading(true)
        const response = await axios.get(`https://api.wsfrc.com/api/dashboard`,{
            headers:{
    ...
    <div className='p-5'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            ...
            <div className='bg-white p-4 rounded-lg shadow-md'>
                ...
                {dashboardData.highesSaleProduct?.name ?(
                    <div className='text-gray-600'>
                        <p><strong>Name:</strong> {dashboardData.highesSaleProduct.name}</p>
                        <p><strong>Category:</strong> {dashboardData.highesSaleProduct.category}</p>
                        <p><strong>Total units sold:</strong> {dashboardData.highesSaleProduct.totalQuantity}</p>
                    </div>
                ): (
                    <p className='text-gray-500'>{dashboardData.highesSaleProduct?.message || 'Loading...'}</p>
                )}
            </div>
